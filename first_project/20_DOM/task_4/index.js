...
createTaskForm.addEventListener('submit', (event) => {
    ...
    if (inputValue && !textOfTasks.includes(inputValue)) {
        ...
        errorSpan.className = 'errorByCreateElement';
        errorSpan.innerHTML = `Название задачи не должно быть пустым! Введите задачу корректно!`
        createTaskForm.insertAdjacentElement('beforeend', errorSpan);
    } else if (textOfTasks.includes(inputValue)) {
        const errorSpan = document.createElement('span');
        errorSpan.className = 'errorByCreateElement';
        errorSpan.innerHTML = `Задача с таким названием уже существует! Исключите повторы!`
        createTaskForm.insertAdjacentElement('beforeend', errorSpan);
    }
})